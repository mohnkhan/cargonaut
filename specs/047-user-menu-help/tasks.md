...
- [ ] T009 (red) [US1] Write failing unit tests for `HelpOverlay` key handling in `crates/cargonaut-ui-tui/src/dialog.rs` — test: `handle_key(Down)` increments scroll_offset when not at bottom; `handle_key(Up)` decrements when not at top, clamps at 0; `handle_key(Home)` resets to 0; `handle_key(End)` sets to max; `handle_key(F1)` / `handle_key(Esc)` returns `HelpAction::Close`; any other key returns `HelpAction::Swallow` (not `Close`); commit `T009(red): failing tests for HelpOverlay::handle_key`
...
- [ ] T012 [P] [US1] Define `pub static HELP_SECTIONS: &[HelpSection]` in `crates/cargonaut-ui-tui/src/dialog.rs` with complete content covering all sections — Navigation, File Operations, Selection, Panels & Modes, Bookmarks, File Attributes, Transfers & Jobs, Theme & Config, User Menu (F2), About; each section must include all live bindings from `design/contracts/keymap.toml` with plain-language descriptions; run `cargo test -p cargonaut-ui-tui` — T008 tests pass; commit `T008/T011/T012(green): HelpSection types + HELP_SECTIONS data`

- [ ] T013 [P] [US1] Add `HelpAction { Close, Swallow }` enum and `HelpOverlay { scroll_offset: u16, total_lines: u16 }` struct with `new()` constructor (computes `total_lines` from `HELP_SECTIONS`) to `crates/cargonaut-ui-tui/src/dialog.rs`

- [ ] T014 [US1] Implement `HelpOverlay::handle_key(&mut self, code: KeyCode) -> HelpAction` in `crates/cargonaut-ui-tui/src/dialog.rs` — Up/Down (1 line), PageUp/PageDown (visible_height lines, pass as param), Home (→0), End (→max), F1/Esc → `Close`, all others → `Swallow`; clamp `scroll_offset` to `[0, total_lines.saturating_sub(visible_height)]`; run `cargo test -p cargonaut-ui-tui` — T009 tests pass; commit `T009/T013/T014(green): HelpOverlay struct + handle_key`
...
- [ ] T032 Run `make ci-local` and fix any clippy warnings, test failures, or doc-link errors introduced by this feature; pay special attention to `#![warn(missing_docs)]` — all new public items in `cargonaut-config` and `cargonaut-ui-tui` must have doc comments

- [ ] T033 [P] Update `README.md` — increment feature count in the "At a Glance" table; add a one-line "Feature 047" entry to the Feature History section; update test count if it changed; update binary size from `scripts/check-binary-size.sh` output

- [ ] T034 [P] Update `Learnings.md` — append a `## Feature 047: User Menu (F2) + Scrollable Help (F1)` section with ≥3 bullet points covering: the `shell-words` tiered-execution decision, the `HELP_BODY → HELP_SECTIONS` migration, the `only_if` timeout strategy, and any surprises encountered during implementation

- [ ] T035 Run `scripts/check-binary-size.sh` after `make build` — verify the release binary did not grow by more than 32 KiB above the Feature 046 baseline (SC-007); record the new size in README.md

- [ ] T036 Run all quickstart.md validation scenarios VS-1 through VS-9 end-to-end in the running TUI; confirm each scenario passes; note any deviations
